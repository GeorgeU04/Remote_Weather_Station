  ...
  if ((TimerObject != NULL) && (TimerExists(TimerObject) == false) &&
    ...
    if (TimerListHead == NULL) {
      ...
      TimerInsertNewHeadTimer(
          TimerObject); /* insert a timeout at now+obj->Timestamp */
    } else {
...
UTIL_TIMER_Status_t
UTIL_TIMER_GetRemainingTime(UTIL_TIMER_Object_t *TimerObject,
                            uint32_t *ElapsedTime) {
  UTIL_TIMER_Status_t ret = UTIL_TIMER_OK;
  if (TimerExists(TimerObject)) {
...
UTIL_TIMER_Object_t *UTIL_TIMER_GetTimerList(void) { return TimerListHead; }
